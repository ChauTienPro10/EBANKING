...
public class AnalysService {
        ...
        private GetAnalysInfoResponse getAnalysInfoByPeriod(String username, long fromDate, long toDate) {
                ...
                Long transactionCount = transactionRepository.countTransactionsInPeriodByUsername(username, fromDate,
                                toDate);
                response.setTransactionCountInPeriodByUsername(transactionCount != null ? transactionCount : 0L);

                // Get largest transaction in period
                response.setTransactionLargestInPeriodByUsername(
                                transactionRepository.getTransactionLargestInPeriodByUsername(username, fromDate,
                                                toDate));

                // Get account that received most transfers
                com.ebanking.transactionService.entity.Account mostTransferAccount = transactionRepository
                                .getMostAccountInfoTransferManyTimeInPeriod(username, fromDate, toDate);
                response.setMostAccountInfoTransferManyTimeInPeriod(mostTransferAccount);

                if (mostTransferAccount != null) {
                        java.util.List<Object[]> statsList = transactionRepository.getTransferStatsByReceiver(username,
                                        mostTransferAccount.getAccountNumber(), fromDate, toDate);

                        if (statsList != null && !statsList.isEmpty()) {
                                Object[] stats = statsList.get(0);
                                response.setMostAccountInfoTransferManyTimeInPeriodCount((Long) stats[0]);
                                response.setMostAccountInfoTransferManyTimeInPeriodTotalAmount((BigDecimal) stats[1]);
                        } else {
                                response.setMostAccountInfoTransferManyTimeInPeriodCount(0);
                                response.setMostAccountInfoTransferManyTimeInPeriodTotalAmount(BigDecimal.ZERO);
                        }
                } else {
                        response.setMostAccountInfoTransferManyTimeInPeriodCount(0);
                        response.setMostAccountInfoTransferManyTimeInPeriodTotalAmount(BigDecimal.ZERO);
                ...
                Long largestTransferAmount = transactionRepository.getTransferHasAmountLargestInPeriod(username,
                                fromDate,
                                toDate);
                response.setTransferHasAmountLargestInPeriod(
                                largestTransferAmount != null ? largestTransferAmount : 0L);

                // Get account that received most total amount
                Long accountWithMostAmount = transactionRepository.getAccountHasBeenTransferWithTheMostAmountInPeriod(
                                username,
                                fromDate, toDate);
                response.setAccountHasBeenTransferWithTheMostAmountInPeriod(
                                accountWithMostAmount != null ? accountWithMostAmount : 0L);

                // Calculate total incoming/outgoing amounts
                try {
                        // Assuming username corresponds to userId and can be parsed to Long
                        Long userId = Long.parseLong(username);
                        com.ebanking.transactionService.entity.Account account = accountRepository.findByUserId(userId);

                        if (account != null) {
                                // Convert timestamps to LocalDateTime
                                LocalDateTime from = java.time.Instant.ofEpochSecond(fromDate)
                                                .atZone(ZoneId.systemDefault()).toLocalDateTime();
                                LocalDateTime to = java.time.Instant.ofEpochSecond(toDate)
                                                .atZone(ZoneId.systemDefault()).toLocalDateTime();

                                BigDecimal incoming = transactionRepository.getTotalIncomingAmountByAccountNumber(
                                                account.getAccountNumber(), from, to);
                                response.setTotalIncomingAmount(incoming != null ? incoming : BigDecimal.ZERO);

                                BigDecimal outgoing = transactionRepository.getTotalOutgoingAmountByAccountNumber(
                                                account.getAccountNumber(), from, to);
                                response.setTotalOutgoingAmount(outgoing != null ? outgoing : BigDecimal.ZERO);
                        } else {
                                response.setTotalIncomingAmount(BigDecimal.ZERO);
                                response.setTotalOutgoingAmount(BigDecimal.ZERO);
                        }
                } catch (NumberFormatException e) {
                        // If username is not a valid Long userId, set amounts to zero or handle
                        // accordingly
                        response.setTotalIncomingAmount(BigDecimal.ZERO);
                        response.setTotalOutgoingAmount(BigDecimal.ZERO);
                }
        ...
        public org.springframework.data.domain.Page<com.ebanking.transactionService.entity.Transaction> getFrequentTransactionsToSameAccount(
                        String username, long fromDate, long toDate, int minTransactions, int timeWindowMinutes,
                        int page,
                        int size) {
                return transactionRepository.findFrequentTransactionsToSameAccount(
                                username, fromDate, toDate, minTransactions, timeWindowMinutes,
