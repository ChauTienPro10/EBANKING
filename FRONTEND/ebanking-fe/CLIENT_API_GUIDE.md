...
const loginResponse = await fetch(
  "http://3.85.17.154:7999/api/admin/auth/login",
  {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
const transactionsResponse = await fetch(
  "http://3.85.17.154:7999/api/transaction-requests?page=0&size=20",
  {
    headers: {
...
const transactionsResponse = await axios.get(
  "http://3.85.17.154:7999/api/transaction-requests",
  {
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
```bash
# 1. Login
TOKEN=$(curl -s -X POST "http://3.85.17.154:7999/api/admin/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"username":"admin","password":"admin@123"}' | jq -r '.token')

# 2. Get transactions
curl -X GET "http://3.85.17.154:7999/api/transaction-requests?page=0&size=20&status=COMPLETED" \
...
# 1. Login
login_response = requests.post('http://3.85.17.154:7999/api/admin/auth/login',
  json={
    'username': 'admin',
    'password': 'admin@123'
  }
...
transactions_response = requests.get('http://3.85.17.154:7999/api/transaction-requests',
